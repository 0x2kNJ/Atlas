...
library MleCheck {
    ...
    function verify(
        ...
        // Iterate through point in REVERSE (as per the binius mlecheck protocol)
        for (uint256 i = 0; i < nVars; i++) {
            // z_i = point[nVars - 1 - i] (reverse iteration)
            uint256 zi = point[nVars - 1 - i];

            // Read [a_1, ..., a_{degree}] from proof (lowest coeff a_0 is truncated)
            ...
            // Recover a_0 = eval - z_i * (a_1 + ... + a_d)
            // In GF(2): a_0 = eval XOR mul(z_i, a_1 XOR a_2)
            uint256 a0;
            if (degree == 2) {
                a0 = eval ^ GF128.mul(zi, a1 ^ a2);
            } else {
                // Higher degrees not currently needed
            ...
            // Evaluate p(r_i) = a_0 + a_1*r_i + a_2*r_i^2 via Horner
    ...
    /// @param coeffs  Coefficients [a_0, a_1, ..., a_d] from lowest to highest degree.
