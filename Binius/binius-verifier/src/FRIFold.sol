...
library FRIFold {
    ...
    function foldChunk(
        ...
        // logLen must allow 4 butterfly layers (each decrements by 1) without going below 6.
        // Valid range: logLen in [10..14] for the Encumber circuit's two oracle folds.
        require(logLen >= 10 && logLen <= 14, "FRIFold: logLen out of range [10,14]");
        require(values.length == 16, "FRIFold: expected 16 coset values");
        require(challenges.length == 4, "FRIFold: expected 4 challenges");
    ...
    function _twiddle(uint256 layer, uint256 blk) private pure returns (uint256 result) {
        result = 0;
        if (blk == 0) return 0;
