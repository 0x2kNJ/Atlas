...
library BiniusCompress {
    ...
    function compress(bytes32 left, bytes32 right) internal pure returns (bytes32 result) {
        assembly {
            // Allocate all working memory from the Solidity free pointer.
            // Layout relative to scratch:
            //   [0x000, 0x040) blockPtr  — 64-byte message block (left || right)
            //   [0x040, 0x840) wPtr      — 64 message-schedule words × 32 bytes each
            //   [0x840, 0x1040) kBase    — 64 round-constant words × 32 bytes each
            //   [0x1040, 0x1060) outPtr  — 32-byte output (written, then copied to result)
            // Total reservation: 0x1060 bytes (4192 bytes). The free pointer is NOT
            // advanced here because this is a pure function; no allocations must be
            // made by callers while this function is executing.
            ...
            // ── Convert to LE bytes (binius format) ──
            // outPtr is placed immediately after kBase region: scratch + 0x40 + 0x800 + 0x800 = scratch + 0x1040
            let outPtr := add(scratch, 0x1040)
